<body>
  <div class="banner md:lg:grid grid-cols-2">
    <div class="banner-illustration w-2/3">
      <img src="./assets/image/illustration-girl.png" alt="">
    </div>
    <div class="banner-text flex flex-col justify-evenly mr-40 font-poppins">
      <h1 class="text-4xl text-wm-darkblue font-bold">Ingin memperbaiki kesehatan mental tapi gak tau mulai darimana?</h1>
      <p>Lorem ipsum dolor sit amet consectetur adipisicing elit. Eum dolores sint, perferendis provident velit quis sequi, est consequatur repellat molestiae error veniam fugiat. Eligendi fugit aliquid quo exercitationem natus aut eius quod. Asperiores nam at nostrum fugit ex possimus, harum doloribus facere corrupti maiores illum reiciendis architecto, dolor tempora quo?</p>
      <div class="banner-button">
        <button class="bg-wm-blue-primary p-2 rounded-lg">Try for free</button>
      </div>
    </div>
  </div>
</body>

</html>